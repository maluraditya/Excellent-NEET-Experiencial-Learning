import { Topic } from '../../../types';

export const TOPICS_11TH: Topic[] = [
    {
        id: 'dimensional-analysis',
        title: 'Dimensional Analysis and Consistency',
        subject: 'Physics',
        unit: 'Unit 1',
        chapter: 'Units and Measurement',
        description: 'Put equations on a dimensional balance scale. Test the principle of homogeneity, watch wrong formulas tilt to red, then derive T = 2π√(l/g) by balancing M, L, T exponents.',
        thumbnailIcon: 'Activity',
        branch: 'Measurement',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-physics/thumb_dimensional_analysis.png',
        youtubeVideoIds: ['bNaRUh8T_wQ']
    },
    {
        id: 'position-velocity-acceleration-graphs',
        title: 'Position, Velocity and Acceleration Graphs',
        subject: 'Physics',
        unit: 'Unit 2',
        chapter: 'Motion in a Straight Line',
        description: 'Link x-t, v-t and a-t graphs to one moving object. Read instantaneous slopes, signed area and the exact kinematic equations for constant acceleration.',
        thumbnailIcon: 'Activity',
        branch: 'Kinematics',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-physics/thumb_position_velocity_acceleration_graphs.png',
        youtubeVideoIds: ['bqf8m7xNvLg']
    },
    {
        id: 'projectile-motion',
        title: 'Projectile Motion & Vector Resolution',
        subject: 'Physics',
        unit: 'Unit 3',
        chapter: 'Motion in a Plane',
        description: 'Launch a projectile and watch its velocity resolve into a constant horizontal part and a gravity-driven vertical part. See the parabola form, find max range at 45°, and read time of flight, height and range live.',
        thumbnailIcon: 'Activity',
        branch: 'Kinematics',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-physics/thumb_projectile_motion.png',
        youtubeVideoIds: ['d_jnEkwCA6I']
    },
    {
        id: 'static-kinetic-friction',
        title: 'Static & Kinetic Friction',
        subject: 'Physics',
        unit: 'Unit 4',
        chapter: 'Laws of Motion',
        description: 'Push a block and watch static friction self-adjust up to its limit μₛN, then drop to the constant kinetic value μₖN once it slides. Tilt an incline to find the angle of repose, tanθ = μₛ.',
        thumbnailIcon: 'Activity',
        branch: 'Dynamics',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-physics/thumb_static_kinetic_friction.png',
        youtubeVideoIds: ['3EbUa5ZDybg']
    },
    {
        id: 'newtons-laws-of-motion',
        title: "Newton's Laws of Motion",
        subject: 'Physics',
        unit: 'Unit 4',
        chapter: 'Laws of Motion',
        description: 'Switch between the three laws on one bench: glide a frictionless puck (inertia), push a mass to see a = F/m, and release two carts to watch equal-opposite forces conserve momentum.',
        thumbnailIcon: 'Activity',
        branch: 'Dynamics',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-physics/thumb_newtons_laws.png',
        youtubeVideoIds: ['825LxfhRJrI']
    },
    {
        id: 'conservation-of-momentum',
        title: 'Conservation of Momentum',
        subject: 'Physics',
        unit: 'Unit 4',
        chapter: 'Laws of Motion',
        description: 'Collide two pucks elastically or inelastically, or fire them apart from rest, and watch the total-momentum bar stay equal before and after — while kinetic energy is conserved only in elastic collisions.',
        thumbnailIcon: 'Activity',
        branch: 'Dynamics',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-physics/thumb_conservation_momentum.png',
        youtubeVideoIds: ['IUKG1EeFrcI']
    },
    {
        id: 'work-energy-theorem',
        title: 'Work-Energy Theorem',
        subject: 'Physics',
        unit: 'Unit 5',
        chapter: 'Work, Energy and Power',
        description: 'Push a block across a track with a force at any angle, against friction, and watch the net-work bar rise in lock-step with the change-in-kinetic-energy bar — proving W_net = ΔKE, with W = Fd cosθ.',
        thumbnailIcon: 'Activity',
        branch: 'Energy',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-physics/thumb_work_energy.png',
        youtubeVideoIds: ['gvoqENvHLE0']
    },
    {
        id: 'conservation-of-angular-momentum',
        title: 'Conservation of Angular Momentum',
        subject: 'Physics',
        unit: 'Unit 6',
        chapter: 'System of Particles and Rotational Motion',
        description: 'Spin a skater and pull the arms in: moment of inertia drops, angular speed rises, but L = Iω stays constant (τ = 0). Watch the L bar hold steady while rotational KE changes.',
        thumbnailIcon: 'Activity',
        branch: 'Rotational',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-physics/thumb_angular_momentum.png',
        youtubeVideoIds: ['ho3VQy4wO4E']
    },
    {
        id: 'centre-of-mass-torque',
        title: 'Centre of Mass and Torque',
        subject: 'Physics',
        unit: 'Unit 6',
        chapter: 'System of Particles and Rotational Motion',
        description: 'Watch two unequal masses orbit their mass-weighted centre, then apply a force to a pivoted beam and explore how force, lever arm and angle determine torque.',
        thumbnailIcon: 'Activity',
        branch: 'Rotational',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-physics/thumb_centre_of_mass_torque.png',
        youtubeVideoIds: ['ENWqHiwX32U']
    },
    {
        id: 'moment-of-inertia',
        title: 'Moment of Inertia',
        subject: 'Physics',
        unit: 'Unit 6',
        chapter: 'System of Particles and Rotational Motion',
        description: 'Apply the same torque to a ring, disc, rod and sphere. See how mass distribution changes moment of inertia, angular acceleration, radius of gyration and rotational kinetic energy.',
        thumbnailIcon: 'Activity',
        branch: 'Rotational',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-physics/thumb_moment_of_inertia.png',
        youtubeVideoIds: ['pTo8sAJKVkA']
    },
    {
        id: 'conservation-mechanical-energy',
        title: 'Conservation of Mechanical Energy',
        subject: 'Physics',
        unit: 'Unit 5',
        chapter: 'Work, Energy and Power',
        description: 'Track kinetic, potential and transferred energy through NCERT ideal free fall, a spring-block oscillator and a clearly labelled non-conservative comparison.',
        thumbnailIcon: 'Activity',
        branch: 'Energy',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-physics/thumb_conservation_mechanical_energy.png',
        youtubeVideoIds: ['BOxcFUpbydA']
    },
    {
        id: 'keplers-laws-planetary-motion',
        title: "Kepler's Laws of Planetary Motion",
        subject: 'Physics',
        unit: 'Unit 7',
        chapter: 'Gravitation',
        description: 'Place the Sun at an ellipse focus, compare equal-area sectors and test T² proportional to a³ with animated planets whose orbital speeds and periods follow Kepler\'s laws.',
        thumbnailIcon: 'Activity',
        branch: 'Gravitation',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-physics/thumb_keplers_laws.png',
        youtubeVideoIds: ['jW2_H6-_72A']
    },
    {
        id: 'mechanical-properties-solids',
        title: 'Mechanical Properties of Solids',
        subject: 'Physics',
        unit: 'Unit 8',
        chapter: 'Mechanical Properties of Solids',
        description: 'Explore elasticity, stress-strain curves, and Young\'s Modulus through interactive tensile testing of various materials.',
        thumbnailIcon: 'Activity', // Lucide icon name to be matched in Dashboard
        branch: 'Solids',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-physics/thumb_solids_1771662772975.png',
        youtubeVideoIds: ['L7gOIFgYl2k']
    },
    {
        id: 'stokes-law',
        title: 'Stokes’ Law and Terminal Velocity',
        subject: 'Physics',
        unit: 'Unit 9',
        chapter: 'Mechanical Properties of Fluids',
        description: "Discover how viscous drag balances gravity to produce terminal velocity. Drop spheres into Water, Oil, and Glycerin and watch force vectors balance in real-time.",
        thumbnailIcon: 'Activity',
        branch: 'Fluids',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-physics/thumb_stokes_law_1775281306204.png',
        youtubeVideoIds: ['L7gOIFgYl2k']
    },
    {
        id: 'fluid-dynamics',
        title: 'Bernoulli’s Principle and its Applications',
        subject: 'Physics',
        unit: 'Unit 9',
        chapter: 'Mechanical Properties of Fluids',
        description: 'Explore the Equation of Continuity and Bernoulli\'s Principle with an interactive Venturi tube and virtual wind tunnel.',
        thumbnailIcon: 'wind', // Will map to 'wind' icon in Dashboard
        branch: 'Fluids',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-physics/thumb_fluids_1771662792973.png',
        youtubeVideoIds: ['eJ_Sx_VP5dY']
    },
    {
        id: 'pascals-law',
        title: 'Fluid Pressure & Pascal\'s Law',
        subject: 'Physics',
        unit: 'Unit 9',
        chapter: 'Mechanical Properties of Fluids',
        description: 'Experience Pascal\'s Law through an interactive hydraulic disc brake. Learn about force multiplication, pressure transmission, and the conservation of energy.',
        thumbnailIcon: 'Activity',
        branch: 'Fluids',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-physics/thumb_pascals_1771662812204.png',
        youtubeVideoIds: ['h2IBnGURgHY']
    },
    {
        id: 'surface-tension',
        title: 'Surface Tension & Capillarity',
        subject: 'Physics',
        unit: 'Unit 9',
        chapter: 'Mechanical Properties of Fluids',
        description: 'Discover how molecular forces create surface tension, how the angle of contact determines wetting behavior, and why liquids rise in narrow tubes.',
        thumbnailIcon: 'Activity',
        branch: 'Fluids',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-physics/thumb_surface_tension.png',
        youtubeVideoIds: ['L7gOIFgYl2k'] // Placeholder
    },
    {
        id: 'carnot-engine',
        title: 'Carnot Engine and Carnot Cycle',
        subject: 'Physics',
        unit: 'Unit 11',
        chapter: 'Thermodynamics',
        description: 'Build the Carnot Cycle step-by-step on a P-V diagram. Explore isothermal and adiabatic processes, and understand the theoretical maximum efficiency of heat engines.',
        thumbnailIcon: 'Activity',
        branch: 'Thermodynamics',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-physics/thumb_carnot_1771662842471.png',
        youtubeVideoIds: ['IMXcixHO9u0']
    },
    {
        id: 'zeroth-law',
        title: 'Zeroth Law of Thermodynamics',
        subject: 'Physics',
        unit: 'Unit 11',
        chapter: 'Thermodynamics',
        description: 'Discover thermal equilibrium and the foundation of temperature. Control adiabatic and diathermic walls between three gas chambers to prove the Zeroth Law.',
        thumbnailIcon: 'Activity',
        branch: 'Thermodynamics',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-physics/thumb_zeroth_law.png',
        youtubeVideoIds: ['L7gOIFgYl2k']
    },
    {
        id: 'thermodynamic-processes',
        title: 'First Law of Thermodynamics and Thermodynamic Processes',
        subject: 'Physics',
        unit: 'Unit 11',
        chapter: 'Thermodynamics',
        description: 'Perform isothermal, adiabatic, isobaric, and isochoric processes on an interactive gas cylinder. Observe how Heat (Q), Work (W), and Internal Energy (ΔU) relate through the First Law.',
        thumbnailIcon: 'Activity',
        branch: 'Thermodynamics',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-physics/thumb_thermo_1771662859751.png',
        youtubeVideoIds: ['w3MEJoTW2Oo']
    },
    {
        id: 'thermal-expansion-calorimetry',
        title: 'Thermal Properties of Matter',
        subject: 'Physics',
        unit: 'Unit 10',
        chapter: 'Thermal Properties of Matter',
        description: 'Investigate how heat alters matter. Observe linear, area, and volume expansion, study the anomalous behavior of water, trace phase changes on a heating curve, and perform virtual calorimetry experiments.',
        thumbnailIcon: 'Activity',
        branch: 'Thermal',
        grade: '11th',
        thumbnailUrl: '/thumbnail-thermal-expansion.png',
        youtubeVideoIds: ['w3MEJoTW2Oo']
    },
    {
        id: 'heat-transfer-blackbody-radiation',
        title: 'Heat Transfer and Blackbody Radiation',
        subject: 'Physics',
        unit: 'Unit 10',
        chapter: 'Thermal Properties of Matter',
        description: 'Compare conduction, convection, and radiation in one thermal lab. Track heat flow, convection currents, Wien peak shift, and blackbody power as temperature changes.',
        thumbnailIcon: 'Activity',
        branch: 'Thermal',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-physics/thumb_blackbody_radiation_1775281143953.png',
        youtubeVideoIds: ['w3MEJoTW2Oo']
    },
    {
        id: 'kinetic-theory',
        title: 'Pressure of an Ideal Gas (Kinetic Theory)',
        subject: 'Physics',
        unit: 'Unit 12',
        chapter: 'Kinetic Theory',
        description: 'Visualize how billions of molecular collisions create gas pressure. Adjust Temperature, Volume, and molecule count to validate P = ⅓nm⟨v²⟩.',
        thumbnailIcon: 'Activity',
        branch: 'Kinetic Theory',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-physics/thumb_kinetic_1771662889485.png',
        youtubeVideoIds: ['k4FvOpHU0jo']
    },
    {
        id: 'mean-free-path',
        title: 'Mean free path',
        subject: 'Physics',
        unit: 'Unit 12',
        chapter: 'Kinetic Theory',
        description: 'Visualize the zig-zag random motion of a tracked gas molecule. Discover how increasing density or molecular size dramatically shortens the mean free path using l = 1/(√2 nπd²).',
        thumbnailIcon: 'Activity',
        branch: 'Kinetic Theory',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-physics/thumb_mean_free_path_1775281125480.png',
        youtubeVideoIds: ['k4FvOpHU0jo']
    },
    {
        id: 'equipartition',
        title: 'Degrees of Freedom and Equipartition of Energy',
        subject: 'Physics',
        unit: 'Unit 12',
        chapter: 'Kinetic Theory',
        description: 'Switch between Monatomic, Diatomic, and Polyatomic gases. Watch how energy partitions equally across translation, rotation, and vibration — explaining why complex gases have higher specific heats.',
        thumbnailIcon: 'Activity',
        branch: 'Kinetic Theory',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-physics/thumb_equipartition_1771662906243.png',
        youtubeVideoIds: ['qyLrCR15rBY']
    },

    {
        id: 'shm-spring',
        title: 'Spring-Mass System and Simple Harmonic Motion',
        subject: 'Physics',
        unit: 'Unit 13',
        chapter: 'Oscillations',
        description: 'Drag a block on a spring and watch SHM unfold. Observe x(t), v(t), a(t) phase shifts, energy conservation KE↔PE, and prove T = 2π√(m/k) is independent of amplitude.',
        thumbnailIcon: 'Activity',
        branch: 'Oscillations',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-physics/thumb_shm_1771662927209.png',
        youtubeVideoIds: ['m3CtyV35s8U']
    },
    {
        id: 'simple-pendulum',
        title: 'Simple Pendulum and Oscillatory Motion',
        subject: 'Physics',
        unit: 'Unit 13',
        chapter: 'Oscillations',
        description: 'Explore the Simple Pendulum — discover that T = 2π√(L/g) depends only on length and gravity, not on mass or amplitude (within small angles). Drag the bob, measure period, and verify SHM.',
        thumbnailIcon: 'Activity',
        branch: 'Oscillations',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-physics/thumb_simple_pendulum_1775281077905.png',
        youtubeVideoIds: ['m3CtyV35s8U']
    },
    {
        id: 'wave-motion',
        title: 'Wave Motion',
        subject: 'Physics',
        unit: 'Unit 14',
        chapter: 'Waves',
        description: 'Visualize how energy travels through a medium without matter transport. Toggle between transverse string waves and longitudinal sound pulses in air.',
        thumbnailIcon: 'Activity',
        branch: 'Waves',
        grade: '11th',
        thumbnailUrl: '/thumbnail-wave-motion.png',
        youtubeVideoIds: ['mQdp4Be9qM8']
    },
    {
        id: 'standing-waves',
        title: 'Superposition, Reflection & Standing Waves',
        subject: 'Physics',
        unit: 'Unit 14',
        chapter: 'Waves',
        description: 'Drive a string at different frequencies. Watch incident and reflected waves superpose into standing waves. Discover nodes, antinodes, and harmonic modes when resonance is hit.',
        thumbnailIcon: 'Activity',
        branch: 'Waves',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-physics/thumb_waves_1771662957415.png',
        youtubeVideoIds: ['mQdp4Be9qM8']
    },
    {
        id: 'hydrogen-spectrum',
        title: 'Hydrogen Spectrum and Energy Levels',
        subject: 'Chemistry',
        unit: 'Unit 2',
        chapter: 'Structure of Atom',
        description: 'Experience quantization by firing photons at a hydrogen atom. Observe electron transitions and the resulting emission spectrum to understand the Bohr model.',
        thumbnailIcon: 'Activity',
        branch: 'Physical Chemistry',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-chemistry/thumb_hydrogen_bohr_generated.png',
        youtubeVideoIds: ['tigyyU8nZWI']
    },
    {
        id: 'atomic-orbitals',
        title: 'Shapes of Atomic Orbitals and Nodal Surfaces',
        subject: 'Chemistry',
        unit: 'Unit 2',
        chapter: 'Structure of Atom',
        description: 'Explore the 3D probability clouds of s, p, and d orbitals. Slice open and visualize radial and angular nodes dictated by quantum mechanics.',
        thumbnailIcon: 'Activity',
        branch: 'Physical Chemistry',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-chemistry/thumb_atomic_orbitals_generated.png',
        youtubeVideoIds: ['nNkw_0c8vY0']
    },
    {
        id: 'electronic-configuration-exchange-energy',
        title: 'Electronic Configuration and Exchange Energy',
        subject: 'Chemistry',
        unit: 'Unit 2',
        chapter: 'Structure of Atom',
        description: 'Build electronic configurations using Aufbau principle, Pauli exclusion principle, and Hund rule, then compare exchange energy and the stability of half-filled and fully-filled subshells.',
        thumbnailIcon: 'Activity',
        branch: 'Physical Chemistry',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-chemistry/thumb_electronic_configuration.svg',
        youtubeVideoIds: ['igEGO8h3N-A']
    },
    {
        id: 'vsepr-theory',
        title: 'Molecular Geometry and Lone Pair Effects (VSEPR)',
        subject: 'Chemistry',
        unit: 'Unit 4',
        chapter: 'Chemical Bonding and Molecular Structure',
        description: 'Understand how electron repulsion forces molecular geometry. Build 3D molecules, alter lone pairs, and visually grasp shapes from bent to octahedral.',
        thumbnailIcon: 'Activity',
        branch: 'Physical Chemistry',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-chemistry/thumb_vsepr_generated.png',
        youtubeVideoIds: ['nxebQZUVvTg']
    },
    {
        id: 'sigma-pi-bonds',
        title: 'Sigma and Pi Bond Formation',
        subject: 'Chemistry',
        unit: 'Unit 4',
        chapter: 'Chemical Bonding and Molecular Structure',
        description: 'Visualize how s and p orbitals overlap head-on or laterally to form Sigma and Pi bonds. Discover why phase matters and why π bonds are weaker than σ bonds.',
        thumbnailIcon: 'Activity',
        branch: 'Physical Chemistry',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-chemistry/thumb_sigma_pi_generated.png',
        youtubeVideoIds: ['pT8nrBrTOm4']
    },
    {
        id: 'isothermal-work',
        title: 'Isothermal Reversible and Irreversible Work',
        subject: 'Chemistry',
        unit: 'Unit 5',
        chapter: 'Chemical Thermodynamics',
        description: 'Simulate piston-cylinder expansion under isothermal conditions. Compare work done in reversible vs irreversible processes with real-time P–V diagrams.',
        thumbnailIcon: 'Activity',
        branch: 'Physical Chemistry',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-chemistry/thumb_isothermal_work_generated.png',
        youtubeVideoIds: ['S4wX2RA9Iw0']
    },
    {
        id: 'heat-work-energy-changes',
        title: 'Heat, Work, and Energy Changes',
        subject: 'Chemistry',
        unit: 'Unit 5',
        chapter: 'Thermodynamics',
        description: 'Track heat, pressure-volume work, and internal energy using the chemistry sign convention. Adjust q, external pressure, and volume change to verify dU = q + w in real time.',
        thumbnailIcon: 'Activity',
        branch: 'Physical Chemistry',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-chemistry/thumb_heat_work_energy.svg',
        youtubeVideoIds: ['S4wX2RA9Iw0']
    },
    {
        id: 'extensive-intensive-properties',
        title: 'Intensive and Extensive Properties',
        subject: 'Chemistry',
        unit: 'Unit 1',
        chapter: 'Some Basic Concepts of Chemistry',
        description: 'Explore how thermodynamic properties behave when you divide a system. Insert a partition, pump gas, and discover which properties depend on system size.',
        thumbnailIcon: 'Activity',
        branch: 'Physical Chemistry',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-chemistry/thumb_extensive_intensive_generated.png',
        youtubeVideoIds: ['hsUeKdjdl1g']
    },
    {
        id: 'mole-concept-limiting-reagent',
        title: 'Mole Concept and Limiting Reagent',
        subject: 'Chemistry',
        unit: 'Unit 1',
        chapter: 'Some Basic Concepts of Chemistry',
        description: 'Mix reactants as moles or mass, watch balanced-equation mole packets react, and identify the limiting reagent, product formed, and excess left behind.',
        thumbnailIcon: 'Activity',
        branch: 'Physical Chemistry',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-chemistry/thumb_mole_concept.svg',
        youtubeVideoIds: ['StxXr1SoUdw']
    },
    {
        id: 'solution-concentration-dilution',
        title: 'Solution Concentration and Dilution',
        subject: 'Chemistry',
        unit: 'Unit 1',
        chapter: 'Some Basic Concepts of Chemistry',
        description: 'Prepare solutions and watch mass %, mole fraction, molarity and molality update together. Switch to dilution mode and verify M₁V₁ = M₂V₂ live.',
        thumbnailIcon: 'Activity',
        branch: 'Physical Chemistry',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-chemistry/thumb_solution_concentration.svg',
        youtubeVideoIds: ['V6-mlh9xRjg']
    },
    {
        id: 'periodic-trends-explorer',
        title: 'Periodic Trends Explorer',
        subject: 'Chemistry',
        unit: 'Unit 3',
        chapter: 'Classification of Elements and Periodicity in Properties',
        description: 'Switch lenses on the periodic table — atomic radius, ionisation enthalpy, electronegativity, metallic character — and watch tiles recolour and resize by NCERT data, with live period and group trend graphs.',
        thumbnailIcon: 'Activity',
        branch: 'Inorganic Chemistry',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-chemistry/thumb_periodic_trends.svg',
        youtubeVideoIds: ['EPbyr769YXk']
    },
    {
        id: 'redox-oxidation-number',
        title: 'Redox and Oxidation Number Visualiser',
        subject: 'Chemistry',
        unit: 'Unit 7',
        chapter: 'Redox Reactions',
        description: 'Assign oxidation numbers using the 7 NCERT rules, classify reactions as combination / decomposition / displacement / disproportionation, and step through the oxidation-number and half-reaction balancing methods on NCERT worked examples (Cr₂O₇²⁻ + SO₃²⁻, MnO₄⁻ + Br⁻, Fe²⁺ + Cr₂O₇²⁻).',
        thumbnailIcon: 'Activity',
        branch: 'Physical Chemistry',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-chemistry/thumb_redox.svg',
        youtubeVideoIds: ['OCZK3BG9fCU']
    },
    {
        id: 'weak-acid-base-ionization',
        title: 'Ionization of Weak Acids/Bases and Equilibrium K',
        subject: 'Chemistry',
        unit: 'Unit 6',
        chapter: 'Equilibrium',
        description: 'Two beakers ionize side-by-side — pick the acid (HF, HCN, CH₃COOH…) and base (NH₃, pyridine, aniline…), drag concentration on a log scale, and watch α, [H⁺], [OH⁻] and pH update live. Switch to conjugate-pair mode for NH₄⁺/NH₃ and confirm Ka × Kb = Kw.',
        thumbnailIcon: 'Activity',
        branch: 'Physical Chemistry',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-chemistry/thumb_weak_acid_base.svg',
        youtubeVideoIds: ['V9-ztfDe1_0']
    },
    {
        id: 'hydrogen-bonding-molecular-interaction',
        title: 'Hydrogen Bonding and Molecular Interaction',
        subject: 'Chemistry',
        unit: 'Unit 4',
        chapter: 'Chemical Bonding and Molecular Structure',
        description: 'Compare molecules with and without H-bonding side-by-side. Watch δ+/δ− partial charges line up into dotted bridges, slide temperature from solid to gas to see bond strength change, and switch to intramolecular mode for o-nitrophenol.',
        thumbnailIcon: 'Activity',
        branch: 'Physical Chemistry',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-chemistry/thumb_hydrogen_bonding.svg',
        youtubeVideoIds: ['3N8aCwu4RbA']
    },
    {
        id: 'buffer-solutions',
        title: 'Buffer Solution and pH Stability',
        subject: 'Chemistry',
        unit: 'Unit 6',
        chapter: 'Equilibrium',
        description: 'Build buffer solutions and test their resistance to pH change. Compare buffered vs unbuffered water and discover when buffers break.',
        thumbnailIcon: 'Activity',
        branch: 'Physical Chemistry',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-chemistry/thumb_buffer_solutions_generated.png',
        youtubeVideoIds: ['8U5tP6GL9wM']
    },
    {
        id: 'le-chatelier-equilibrium',
        title: 'Effect of Concentration on Equilibrium',
        subject: 'Chemistry',
        unit: 'Unit 6',
        chapter: 'Equilibrium',
        description: 'Manipulate the Iron-Thiocyanate equilibrium with reagent droppers. Watch solution color shift as Qc races to match Kc in real time.',
        thumbnailIcon: 'Activity',
        branch: 'Physical Chemistry',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-chemistry/thumb_le_chatelier_generated.png',
        youtubeVideoIds: ['bZFN6-B46xk']
    },
    {
        id: 'qualitative-analysis-organic',
        title: 'Qualitative Analysis of Elements in Organic Compounds',
        subject: 'Chemistry',
        unit: 'Unit 8',
        chapter: 'Organic Chemistry: Some Basic Principles and Techniques',
        description: 'Master Lassaigne\'s Test. Experience why covalent organic compounds must undergo sodium fusion before testing for Nitrogen, Sulphur, and Halogens.',
        thumbnailIcon: 'Activity',
        branch: 'Organic Chemistry',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-chemistry/thumb_qualitative_analysis_generated.png',
        youtubeVideoIds: ['kv5lWm0Afwg']
    },
    {
        id: 'quantitative-analysis-organic',
        title: 'Quantitative Analysis and Elemental Composition',
        subject: 'Chemistry',
        unit: 'Unit 8',
        chapter: 'Organic Chemistry: Some Basic Principles and Techniques',
        description: 'Build Liebig\'s combustion train, trap invisible gases, and deduce the empirical formula from mass data using the Liebig method.',
        thumbnailIcon: 'Activity',
        branch: 'Organic Chemistry',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-chemistry/thumb_quantitative_analysis_generated.png',
        youtubeVideoIds: ['4t0J4GhiDlw']
    },
    {
        id: 'purification-techniques',
        title: 'Purification Techniques',
        subject: 'Chemistry',
        unit: 'Unit 8',
        chapter: 'Organic Chemistry: Some Basic Principles and Techniques',
        description: 'Choose the right purification method for an organic mixture. Compare sublimation, crystallisation, distillation, differential extraction, and chromatography using physical-property clues.',
        thumbnailIcon: 'Activity',
        branch: 'Organic Chemistry',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-chemistry/thumb_purification.svg',
        youtubeVideoIds: ['blc2hr6e5RI']
    },
    {
        id: 'structural-isomerism-molecular-properties',
        title: 'Structural Isomerism and Molecular Properties',
        subject: 'Chemistry',
        unit: 'Unit 8',
        chapter: 'Organic Chemistry: Some Basic Principles and Techniques',
        description: 'Compare chain, position, functional group isomerism, and metamerism. See how the same molecular formula can produce different connectivity, boiling points, polarity, and molecular properties.',
        thumbnailIcon: 'Activity',
        branch: 'Organic Chemistry',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-chemistry/thumb_structural_isomerism.svg',
        youtubeVideoIds: ['pg6GWz3zXas']
    },
    {
        id: 'ethane-conformations',
        title: 'Conformations of Ethane',
        subject: 'Chemistry',
        unit: 'Unit 9',
        chapter: 'Hydrocarbons',
        description: 'Rotate the C-C bond of ethane in real-time. Explore staggered vs eclipsed conformations through Newman, Sawhorse, and 3D projections with a live energy graph.',
        thumbnailIcon: 'Activity',
        branch: 'Organic Chemistry',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-chemistry/thumb_ethane_conformations_generated.png',
        youtubeVideoIds: ['kdurI-JHj2M']
    },
    {
        id: 'stereoisomerism-geometrical',
        title: 'Stereoisomerism and Molecular Arrangement',
        subject: 'Chemistry',
        unit: 'Unit 9',
        chapter: 'Hydrocarbons',
        description: 'Discover why double bonds lock rotation. Build cis and trans isomers, visualize dipole vector addition/cancellation, and compare physical properties.',
        thumbnailIcon: 'Activity',
        branch: 'Organic Chemistry',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-chemistry/thumb_geometrical_isomerism_generated.png',
        youtubeVideoIds: ['cBd4n_lYsbQ']
    },
    {
        id: 'binomial-nomenclature',
        title: 'Binomial Nomenclature & Taxonomic Hierarchy',
        subject: 'Biology',
        unit: 'Unit 1',
        chapter: 'The Living World',
        description: 'Learn to correctly format scientific names using Binomial rules and visualize how organisms are grouped into higher ranks based on shared characters.',
        thumbnailIcon: 'Activity',
        branch: 'General Biology',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_binomial_nomenclature.png',
        youtubeVideoIds: ['EqA5hv-Qu6c']
    },
    {
        id: 'five-kingdom-classification',
        title: 'Five Kingdom Classification',
        subject: 'Biology',
        unit: 'Unit 2',
        chapter: 'Biological Classification',
        description: 'Discover the defining characteristics of the five kingdoms by scanning mystery organisms and placing them correctly based on R.H. Whittaker\'s criteria.',
        thumbnailIcon: 'Activity',
        branch: 'General Biology',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_five_kingdom.png',
        youtubeVideoIds: ['2cb_HS5mSjI']
    },
    {
        id: 'algae',
        title: 'Algae Classes & Life Cycles',
        subject: 'Biology',
        unit: 'Unit 3',
        chapter: 'Plant Kingdom',
        description: 'Observe and compare the cellular structure, pigments, stored food, and reproductive motility of Chlorophyceae, Phaeophyceae, and Rhodophyceae.',
        thumbnailIcon: 'Activity',
        branch: 'Botany',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_algae.svg',
        youtubeVideoIds: ['i9bBrCf3nhQ']
    },
    {
        id: 'bryophytes-pteridophytes',
        title: 'Bryophytes vs Pteridophytes',
        subject: 'Biology',
        unit: 'Unit 3',
        chapter: 'Plant Kingdom',
        description: 'Compare mosses and ferns to understand why bryophytes remain small, why pteridophytes grow taller, and how dominance shifts from gametophyte to sporophyte.',
        thumbnailIcon: 'Activity',
        branch: 'Botany',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_bryophytes_pteridophytes.svg',
        youtubeVideoIds: ['egetkqOSYM8']
    },
    {
        id: 'gymnosperms-angiosperms',
        title: 'Gymnosperms vs Angiosperms',
        subject: 'Biology',
        unit: 'Unit 3',
        chapter: 'Plant Kingdom',
        description: 'Compare naked and enclosed seeds, visualise the pollen tube journey, and witness Double Fertilisation — unique to Angiosperms — step by step.',
        thumbnailIcon: 'Activity',
        branch: 'Botany',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_gymnosperms_angiosperms.svg',
        youtubeVideoIds: ['oIeN8V5JDkM']
    },
    {
        id: 'animal-kingdom-non-chordates',
        title: 'Non-Chordate Phyla Body Plans',
        subject: 'Biology',
        unit: 'Unit 4',
        chapter: 'Animal Kingdom',
        description: 'Study the sponge canal system and the polyp-medusa life cycle of cnidarians through a simple marine observation simulation.',
        thumbnailIcon: 'Activity',
        branch: 'Zoology',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_animal_kingdom_non_chordates.svg',
        youtubeVideoIds: ['Uc-Gi7fECo8']
    },
    {
        id: 'morphology-flowering-plants',
        title: 'Root-Stem-Leaf Modifications',
        subject: 'Biology',
        unit: 'Unit 5',
        chapter: 'Morphology of Flowering Plants',
        description: 'Compare tap root and fibrous root systems, explore alternate, opposite, and whorled phyllotaxy, and understand tendrils and spines as survival modifications.',
        thumbnailIcon: 'Activity',
        branch: 'Botany',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_morphology_flowering_plants.svg',
        youtubeVideoIds: ['1K_G4UXc3s0']
    },
    {
        id: 'anatomy-flowering-plants',
        title: 'Tissue Systems & Secondary Growth',
        subject: 'Biology',
        unit: 'Unit 6',
        chapter: 'Anatomy of Flowering Plants',
        description: 'Visualize epidermal, ground, and vascular tissue systems, and understand how vascular cambium drives secondary growth and wood formation in dicot stems.',
        thumbnailIcon: 'Activity',
        branch: 'Botany',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_anatomy_flowering_plants_generated.png',
        youtubeVideoIds: ['Tl4bQEWN7cQ']
    },
    {
        id: 'animal-tissues',
        title: 'Epithelial-Connective-Muscle Tissues',
        subject: 'Biology',
        unit: 'Unit 7',
        chapter: 'Structural Organisation in Animals',
        description: 'Test how single-layer epithelium tears under friction while compound epithelium holds firm, and compare the pliable cartilage matrix with the rigid bone matrix under load.',
        thumbnailIcon: 'Activity',
        branch: 'Zoology',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_animal_tissues.svg',
        youtubeVideoIds: ['1i_iJBcB51I']
    },
    {
        id: 'chordata',
        title: 'Chordate Classes & Vertebrate Features',
        subject: 'Biology',
        unit: 'Unit 4',
        chapter: 'Animal Kingdom',
        description: 'Explore the four defining chordate features, witness the notochord transform into the vertebral column, and compare heart chambers across vertebrate classes.',
        thumbnailIcon: 'Activity',
        branch: 'Zoology',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_chordata.svg',
        youtubeVideoIds: ['ZNHTKVo4QWg']
    },
    {
        id: 'frogs',
        title: 'Frog Organ Systems & Anatomy',
        subject: 'Biology',
        unit: 'Unit 7',
        chapter: 'Structural Organisation in Animals',
        description: 'Explore frog anatomy on a real dissection photograph. Select any organ-system — digestive, circulatory, respiratory, excretory, nervous or reproductive — to spotlight its organs and watch them work with live animated overlays.',
        thumbnailIcon: 'Activity',
        branch: 'Zoology',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_frogs_generated.png',
        youtubeVideoIds: ['9zKOXDDcjSQ']
    },
    {
        id: 'cell-membrane-transport',
        title: 'Fluid Mosaic Model & Transport',
        subject: 'Biology',
        unit: 'Unit 8',
        chapter: 'Cell: The Unit of Life',
        description: 'Visualize the phospholipid bilayer as a fluid mosaic. Test how neutral solutes, polar molecules, and ions cross by simple diffusion, facilitated diffusion, or ATP-driven active transport.',
        thumbnailIcon: 'Activity',
        branch: 'Cell Biology',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_cell_membrane_generated.png',
        youtubeVideoIds: ['ipa1vmQ7H_4']
    },
    {
        id: 'biomolecules',
        title: 'Carbohydrates-Lipids-Proteins-Nucleic Acids',
        subject: 'Biology',
        unit: 'Unit 9',
        chapter: 'Biomolecules',
        description: 'Build amino acids as substituted methanes, observe zwitterion behaviour with pH, and assemble triglycerides by esterifying fatty acids with glycerol. Connect monomers to proteins, polysaccharides, and nucleic acids.',
        thumbnailIcon: 'Activity',
        branch: 'Biochemistry',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_biomolecules_generated.png',
        youtubeVideoIds: ['rQyWJIn1HYE']
    },
    {
        id: 'enzymes',
        title: 'Enzyme-Substrate Complex & Catalysis',
        subject: 'Biology',
        unit: 'Unit 9',
        chapter: 'Biomolecules',
        description: 'Explore active sites, ES complex formation, induced fit, activation energy, and the Vmax plateau. Adjust substrate concentration, add enzymes, and use inhibitors to understand enzyme saturation.',
        thumbnailIcon: 'Activity',
        branch: 'Biochemistry',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_enzymes_generated.png',
        youtubeVideoIds: ['yk14dOOvwMk']
    },
    {
        id: 'cell-cycle-regulation',
        title: 'Cell Cycle Checkpoints & Regulation',
        subject: 'Biology',
        unit: 'Unit 10',
        chapter: 'Cell Cycle and Cell Division',
        description: 'Explore the control logic of G1, S, G2, M and G0. Use nutrient signals, environment request, DNA repair, and checkpoint gates to understand coordinated cell-cycle regulation.',
        thumbnailIcon: 'Activity',
        branch: 'Cell Biology',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_cell_cycle_generated.png',
        youtubeVideoIds: ['VLJF8Pf8spw']
    },
    {
        id: 'mitosis-vs-meiosis-stages',
        title: 'Mitosis vs Meiosis Stages',
        subject: 'Biology',
        unit: 'Unit 10',
        chapter: 'Cell Cycle and Cell Division',
        description: 'Compare mitosis and meiosis I stage by stage. Observe single-file versus bivalent alignment, centromere splitting, crossing over, and how chromosome number changes from 2n to n.',
        thumbnailIcon: 'Activity',
        branch: 'Cell Biology',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_mitosis_meiosis_stages_generated.png',
        youtubeVideoIds: ['kQu6Yfrr6j0']
    },
    {
        id: 'photosynthesis-light-reaction',
        title: 'Light Reaction & Photophosphorylation',
        subject: 'Biology',
        unit: 'Unit 11',
        chapter: 'Photosynthesis in Higher Plants',
        description: 'Understand how light energy drives PS II and PS I, water splitting, oxygen release, electron transport, proton-gradient formation, ATP synthesis, and NADPH production.',
        thumbnailIcon: 'Activity',
        branch: 'Plant Physiology',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_photosynthesis_light_reaction_generated.png',
        youtubeVideoIds: ['NqCmzk4Z9LA']
    },
    {
        id: 'calvin-cycle-c3-c4-pathways',
        title: 'Calvin Cycle & C3-C4 Pathways',
        subject: 'Biology',
        unit: 'Unit 11',
        chapter: 'Photosynthesis in Higher Plants',
        description: 'Compare the Calvin cycle, C3 pathway, C4 pathway, and photorespiration. See how PEPcase and bundle sheath cells help C4 plants reduce energy waste in hot conditions.',
        thumbnailIcon: 'Activity',
        branch: 'Plant Physiology',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_calvin_cycle_c3_c4_generated.png',
        youtubeVideoIds: ['0UzMaoaXKaM']
    },
    {
        id: 'respiration-in-plants',
        title: 'Glycolysis-TCA Cycle-Fermentation',
        subject: 'Biology',
        unit: 'Unit 12',
        chapter: 'Respiration in Plants',
        description: 'Process glucose through glycolysis, switch oxygen on or off, compare fermentation with aerobic respiration, and track ATP, NADH, FADH2, and CO2 yield.',
        thumbnailIcon: 'Activity',
        branch: 'Plant Physiology',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_electron_transport_chain.svg',
        youtubeVideoIds: ['pNzFYBA2Ofg']
    },
    {
        id: 'electron-transport-chain',
        title: 'Electron Transport Chain & ATP Yield',
        subject: 'Biology',
        unit: 'Unit 12',
        chapter: 'Respiration in Plants',
        description: 'Watch electrons travel through Complexes I-IV, protons pump into the intermembrane space, and ATP synthase spin to make ATP. Toggle inhibitors (rotenone, cyanide, uncoupler) and O₂ supply to see why oxygen is the final hydrogen acceptor.',
        thumbnailIcon: 'Activity',
        branch: 'Plant Physiology',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_respiration_in_plants.png',
        youtubeVideoIds: ['LQmTKxI4Wn4']
    },
    {
        id: 'plant-hormones-tropisms',
        title: 'Plant Hormones & Tropisms',
        subject: 'Biology',
        unit: 'Unit 12',
        chapter: 'Plant Growth and Development',
        description: 'Run Darwin\'s coleoptile phototropism, demonstrate apical dominance with auxin and cytokinin, and watch ethylene ripen a bowl of fruit. Drag any of the five NCERT PGRs onto the stage and watch the canonical effect.',
        thumbnailIcon: 'Sprout',
        branch: 'Plant Physiology',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_plant_hormones_tropisms.svg',
        youtubeVideoIds: ['zm1YQxzHpfM']
    },
    {
        id: 'pulmonary-ventilation-gas-exchange',
        title: 'Pulmonary Ventilation & Gas Exchange',
        subject: 'Biology',
        unit: 'Unit 13',
        chapter: 'Breathing and Exchange of Gases',
        description: 'Follow the full O₂ journey from alveolus to tissue. Adjust breath depth and rate, simulate altitude and exercise, and watch the oxyhaemoglobin dissociation curve cursor shift in real time.',
        thumbnailIcon: 'Activity',
        branch: 'Human Physiology',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_pulmonary_ventilation_gas_exchange.svg',
        youtubeVideoIds: ['8VUVBgfNwkQ']
    },
    {
        id: 'blood-groups-coagulation',
        title: 'Blood Groups & Coagulation Cascade',
        subject: 'Biology',
        unit: 'Unit 13',
        chapter: 'Body Fluids and Circulation',
        description: 'Two tabs: pour donor blood into a recipient and watch antigen-antibody clumping live, or open a cut on a vessel and watch the platelet → thrombokinase → thrombin → fibrin cascade weave a clot.',
        thumbnailIcon: 'Droplets',
        branch: 'Human Physiology',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_blood_groups_coagulation.svg',
        youtubeVideoIds: ['SWR98jAMkgQ']
    },
    {
        id: 'cardiac-cycle-ecg',
        title: 'Cardiac Cycle & ECG Waves',
        subject: 'Biology',
        unit: 'Unit 13',
        chapter: 'Body Fluids and Circulation',
        description: 'See the SA node fire, the conduction wave race across the heart, valves snap with lub-dub, and the P-QRS-T trace scroll past in lockstep with each phase of the 0.8 s cardiac cycle.',
        thumbnailIcon: 'Activity',
        branch: 'Human Physiology',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_cardiac_cycle_ecg.svg',
        youtubeVideoIds: ['RYZ4daFwMa8']
    },
    {
        id: 'glomerular-filtration-reabsorption',
        title: 'Glomerular Filtration & Reabsorption',
        subject: 'Biology',
        unit: 'Unit 13',
        chapter: 'Excretory Products and their Elimination',
        description: 'See ultrafiltration squeeze plasma through podocyte slit pores, watch the PCT brush border actively reabsorb glucose/amino acids/Na⁺, and crank plasma glucose to spill it into the urine.',
        thumbnailIcon: 'Activity',
        branch: 'Human Physiology',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_glomerular_filtration_reabsorption.svg',
        youtubeVideoIds: ['9_h0ZXx1lFw']
    },
    {
        id: 'countercurrent-multiplier',
        title: 'Countercurrent Multiplier System',
        subject: 'Biology',
        unit: 'Unit 13',
        chapter: 'Excretory Products and their Elimination',
        description: 'Build the 300 → 1200 mOsmolL⁻¹ medullary gradient by sending filtrate down the descending limb and salt out of the ascending limb. Toggle ADH and watch urine concentration change in real time.',
        thumbnailIcon: 'Droplets',
        branch: 'Human Physiology',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_countercurrent_multiplier.svg',
        youtubeVideoIds: ['o6EN2SF_taE']
    },
    {
        id: 'sliding-filament-theory',
        title: 'Sliding Filament Theory',
        subject: 'Biology',
        unit: 'Unit 13',
        chapter: 'Locomotion and Movement',
        description: 'Trigger a motor-neuron action potential, watch Ca²⁺ release expose actin binding sites, and see myosin heads cycle to shorten a sarcomere. Cut ATP and witness rigor mortis live.',
        thumbnailIcon: 'Activity',
        branch: 'Human Physiology',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_sliding_filament_theory.svg',
        youtubeVideoIds: ['nTZnBdeIb5c']
    },
    {
        id: 'action-potential',
        title: 'Action Potential Generation',
        subject: 'Biology',
        unit: 'Unit 13',
        chapter: 'Neural Control and Coordination',
        description: 'Click anywhere on an axon to fire an AP. Watch Na⁺ rush in, polarity flip, K⁺ rush out, and the wave propagate. Block channels, disable the Na-K pump, or add myelin to see saltatory conduction.',
        thumbnailIcon: 'Zap',
        branch: 'Human Physiology',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_action_potential.svg',
        youtubeVideoIds: ['iBDXOt_uHTQ']
    },
    {
        id: 'synaptic-transmission',
        title: 'Synaptic Transmission Mechanism',
        subject: 'Biology',
        unit: 'Unit 13',
        chapter: 'Neural Control and Coordination',
        description: 'Watch the full 6-step chemical synapse: AP arrival → Ca²⁺ in → vesicle fusion → neurotransmitter release → receptor binding → post-synaptic potential. Toggle to electrical synapses and compare.',
        thumbnailIcon: 'Zap',
        branch: 'Human Physiology',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_synaptic_transmission.svg',
        youtubeVideoIds: ['cZwb8zqAPXc']
    },
    {
        id: 'endocrine-glands-hormones',
        title: 'Endocrine Glands & Hormone Actions',
        subject: 'Biology',
        unit: 'Unit 13',
        chapter: 'Chemical Coordination and Integration',
        description: 'Click any endocrine gland on a human silhouette to release its hormones to their targets. Run stress, fed, fasting, cold and puberty scenarios to see multiple glands cooperate.',
        thumbnailIcon: 'Activity',
        branch: 'Human Physiology',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_endocrine_glands_hormones.svg',
        youtubeVideoIds: ['SRE_SOazgVw']
    },
    {
        id: 'hormone-action-axis',
        title: 'Mechanism of Hormone Action & Tropic Axis',
        subject: 'Biology',
        unit: 'Unit 13',
        chapter: 'Chemical Coordination and Integration',
        description: 'Contrast peptide hormones (membrane receptors → second messengers) vs steroid hormones (intracellular receptors → gene expression). Trigger HPA / HPT / HPG axes and break each one with NCERT-named disorders.',
        thumbnailIcon: 'Activity',
        branch: 'Human Physiology',
        grade: '11th',
        thumbnailUrl: '/images/thumbnails/11th-biology/thumb_hormone_action_axis.svg',
        youtubeVideoIds: ['Rj9sx7SbJOY']
    }
];
